...
float Notification::m_spawnPositionY = 0.f;
...
{
	setColor(cpp3ds::Color(0, 0, 0, 0));
	setTextColor(cpp3ds::Color(255, 255, 255, 0));
	m_destinationY = getPosition().y;
	move(0, 10.f);
	TweenEngine::Tween::to(*this, POSITION_Y, 0.5f).target(m_destinationY).start(m_tweenManager);
	TweenEngine::Tween::to(*this, COLOR_ALPHA, 0.5f).target(170.f).start(m_tweenManager);
